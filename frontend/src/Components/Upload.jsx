...
const Upload = ({ setOpen }) => {
  ...
  const uploadFile = (file,urlType) => {
    ...
    uploadTask.on('state.changed',(snapshot)=>{
      // Get task progres. including the number of bytes uploaded and the total number of bytes
  ...
  const handleUpload = async (e)=>{
    ...
    setOpen(false)
    res.status === 200 && navigate(`/video/${res.data.savedVideo._id}`)
  }
  ...
  return (
    <Container>
      <Wrapper>
        ...
        <Label>Image:</Label>
        {imgPerc>0 ?("Uploading: "+imgPerc +"%"):(<Input type='file' placeholder='image/*' onChange={e => setImg(e.target.files[0])} />)}
